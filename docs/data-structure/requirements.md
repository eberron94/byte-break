...
| `"history"`                                | `key`           | Checks how many times an action/event has occurred. Accepts `min`, `max`, and `target` (`"player"` or `"byte"`).    |
...
| `"hediff"`                                 | `id`            | Checks the stack count of a status effect. Accepts `minStacks`, `maxStacks`, and `target` (`"player"` or `"byte"`). |
| `"property"`                               | `key`           | Checks root properties on the object (like `isAsleep`). Accepts `value` and `target` (`"player"` or `"byte"`).      |
